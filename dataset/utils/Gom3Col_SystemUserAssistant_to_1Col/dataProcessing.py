...
def process_data_for_fine_tuning(input_file, output_dir):
    ...
    # Get input filename without extension for output files
    input_filename = os.path.splitext(os.path.basename(input_file))[0]
    
    # Save dataset in different formats
    # 1. As .jsonl file
    jsonl_path = os.path.join(output_dir, f"{input_filename}_training_data.jsonl")
    with open(jsonl_path, 'w', encoding='utf-8') as f:
        for item in dataset_dict:
            f.write(json.dumps(item, ensure_ascii=False) + '\n')
    
    # 2. Save as HuggingFace dataset
    dataset.save_to_disk(os.path.join(output_dir, f"{input_filename}_hf_dataset"))
    
    # 3. Create Excel file with single "conversations" column
    conversations_df = pd.DataFrame({
        "conversations": conversations_list
    })
    conversations_df.to_excel(os.path.join(output_dir, f"{input_filename}_conversations_format.xlsx"), index=False)
    
    # 4. Create Excel file with original columns plus conversations column
    combined_df = df.copy()
    combined_df['conversations'] = conversations_list
    combined_df.to_excel(os.path.join(output_dir, f"{input_filename}_output.xlsx"), index=False)
    
    # 5. Also export as CSV with original format
    formatted_df = pd.DataFrame({
        "system_prompt": df["system_prompt"],
        "user_input": df["user_input"],
        "assistant_response": df["assistant_response"]
    })
    formatted_df.to_csv(os.path.join(output_dir, f"{input_filename}_formatted_data.csv"), index=False)
...
if __name__ == "__main__":
    # Example usage
    input_file = "Only_Data_system_user_assistant_v1.1.xlsx"  # Use the Excel file in current directory
